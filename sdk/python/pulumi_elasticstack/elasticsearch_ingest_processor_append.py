...
__all__ = [
    'ElasticsearchIngestProcessorAppendResult',
    'AwaitableElasticsearchIngestProcessorAppendResult',
    'elasticsearch_ingest_processor_append',
    'elasticsearch_ingest_processor_append_output',
]

@pulumi.output_type
class ElasticsearchIngestProcessorAppendResult:
    """
...
class AwaitableElasticsearchIngestProcessorAppendResult(ElasticsearchIngestProcessorAppendResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return ElasticsearchIngestProcessorAppendResult(
...
def elasticsearch_ingest_processor_append(allow_duplicates: Optional[bool] = None,
                                          ...
                                          opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableElasticsearchIngestProcessorAppendResult:
    """
    ...
    :param str media_type: The media type for encoding value. Applies only when value is a template snippet. Must be one of `application/json`, `text/plain`, or `application/x-www-form-urlencoded`.
    ...
    __ret__ = pulumi.runtime.invoke('elasticstack:index/elasticsearchIngestProcessorAppend:ElasticsearchIngestProcessorAppend', __args__, opts=opts, typ=ElasticsearchIngestProcessorAppendResult).value

    return AwaitableElasticsearchIngestProcessorAppendResult(
...
@_utilities.lift_output_func(elasticsearch_ingest_processor_append)
def elasticsearch_ingest_processor_append_output(allow_duplicates: Optional[pulumi.Input[Optional[bool]]] = None,
                                                 ...
                                                 opts: Optional[pulumi.InvokeOptions] = None) -> pulumi.Output[ElasticsearchIngestProcessorAppendResult]:
    """
    Helper data source to which can be used to create a processor to append one or more values to an existing array if the field already exists and it is an array.
    Converts a scalar to an array and appends one or more values to it if the field exists and it is a scalar. Creates an array containing the provided values if the field doesn’t exist.
