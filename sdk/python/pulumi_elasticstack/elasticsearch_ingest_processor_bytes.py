...
__all__ = [
    'ElasticsearchIngestProcessorBytesResult',
    'AwaitableElasticsearchIngestProcessorBytesResult',
    'elasticsearch_ingest_processor_bytes',
    'elasticsearch_ingest_processor_bytes_output',
]

@pulumi.output_type
class ElasticsearchIngestProcessorBytesResult:
    """
...
class AwaitableElasticsearchIngestProcessorBytesResult(ElasticsearchIngestProcessorBytesResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return ElasticsearchIngestProcessorBytesResult(
...
def elasticsearch_ingest_processor_bytes(description: Optional[str] = None,
                                         ...
                                         opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableElasticsearchIngestProcessorBytesResult:
    """
    ...
    __ret__ = pulumi.runtime.invoke('elasticstack:index/elasticsearchIngestProcessorBytes:ElasticsearchIngestProcessorBytes', __args__, opts=opts, typ=ElasticsearchIngestProcessorBytesResult).value

    return AwaitableElasticsearchIngestProcessorBytesResult(
...
@_utilities.lift_output_func(elasticsearch_ingest_processor_bytes)
def elasticsearch_ingest_processor_bytes_output(description: Optional[pulumi.Input[Optional[str]]] = None,
                                                ...
                                                opts: Optional[pulumi.InvokeOptions] = None) -> pulumi.Output[ElasticsearchIngestProcessorBytesResult]:
    """
    Helper data source to which can be used to create a processor to convert a human readable byte value (e.g. 1kb) to its value in bytes (e.g. 1024). If the field is an array of strings, all members of the array will be converted.
